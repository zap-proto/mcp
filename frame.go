...
//
// A Response with neither Result nor Err renders `"result":null`, which is what
// JSON-RPC requires of a call that succeeded and returned nothing: the member
// must be present, and omitting it makes the message a malformed response
// rather than a void one.
