...
// A Response with no id carries `"id":null`, which is what JSON-RPC requires of
// an answer to a request whose id could not be read. Omitting the member makes
// it a notification-shaped object that no client will correlate.
func TestJSON_ParseErrorCarriesNullID(t *testing.T) {
	b, err := json.Marshal(Frame{Kind: Response, Err: &Error{Code: CodeParse, Message: "parse error"}})
	if err != nil {
		t.Fatalf("MarshalJSON: %v", err)
	}
	var m map[string]json.RawMessage
	...
	if got, ok := m["id"]; !ok || string(got) != "null" {
		t.Errorf("parse-error response = %s, want a null id member", b)
	}
}
