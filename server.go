...
func (s *Server) Serve(ln net.Listener) error {
	...
	s.mu.Lock()
	// A Close that arrived BEFORE the accept loop started saw no listener and
	// closed nothing. Refusing to start here is the other half of that window:
	// either Close found the listener and closed it, or this finds the flag and
	// never accepts. Without both halves a shutdown raced against startup —
	// which is every short-lived server, and every test — leaves a goroutine
	// blocked in Accept on a listener nobody will ever close.
	if s.closed {
		s.mu.Unlock()
		return ln.Close()
	}
